...
def update_review(review_id: str, review: ReviewUpdate, user_id: str = Depends(get_current_user)):
...
def delete_review(review_id: str, user_id: str = Depends(get_current_user)):
    supabase.table("reviews").delete().eq("id", review_id).eq("user_id", user_id).execute()
    return {"message": "Review deleted successfully"}

@router.post("/{review_id}/like")
def like_review(review_id: str, user_id: str = Depends(get_current_user)):
...
def unlike_review(review_id: str, user_id: str = Depends(get_current_user)):
...
def add_comment(review_id: str, body: str, user_id: str = Depends(get_current_user)):
...
def delete_comment(review_id: str, comment_id: str, user_id: str = Depends(get_current_user)):
    supabase.table("review_comments").delete().eq("id", comment_id).eq("user_id", user_id).execute()
